...
void main() {
    ...
    if (inData.flags == f_CIRCLE_FILLED || inData.flags == f_CIRCLE_OUTLINE || inData.flags == f_CIRCLE_FILLED_SOFT) {
    ...
    } else if (inData.flags == f_RECTANGLE_FILLED || inData.flags == f_RECTANGLE_OUTLINE) {
        ...
        float dis = abs(uv.x - uv.y) / (inData.borderWidth+inData.borderWidth*abs(inData.deform));
        if (dis > 0.5) {
            discard;
        }
        f_color = inData.fillColor;
        if (dis > 0.1) f_color[3] = 2.5 * (0.5 - dis);
    }
}
